...
read_driver_data <- function(file_name, metric = TRUE) {
  ...
  colnames(scc_follow_info) <- c(
    "LV_ID", "LV_spacing_ft", "LV_time_gap_s", "LV_frspacing_ft",
  ...
  combined_df <- cbind(
    ind_vec_df, vds_chassis_cg_position, turn_signal_info,
  ...
  if (metric) {
    ...
    combined_df <- combined_df %>%
      dplyr::select(-c(
        ED_brake_pedal_force_lbs, ED_speed_mph, LV_spacing_ft,
    ...
    combined_df <- combined_df %>%
      dplyr::select(-c(
        ED_brake_pedal_force_kg, ED_speed_mps, LV_spacing_m,
